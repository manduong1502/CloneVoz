    ...
    <div className="flex bg-[var(--voz-surface)] border border-[var(--voz-link)] rounded-sm text-[13px] overflow-hidden w-fit">
      ...
      {pageNumbers.map(number => (
        <Link 
          ...
          className={`px-3 py-[6px] border-r border-[var(--voz-border)] hover:bg-[var(--voz-border)] cursor-pointer ${currentPage === number ? 'bg-[#183254] font-medium hover:bg-[#183254]' : 'text-[var(--voz-link)]'}`}
          style={currentPage === number ? { color: 'white' } : undefined}
        >
          {number}
        </Link>
      ))}
