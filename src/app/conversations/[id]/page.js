...
export default async function ConversationDetailPage({ params }) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-4">
        <h1 className="text-[22px] font-normal leading-tight text-[#141414] mb-[2px]">{conversation.title}</h1>
        <div className="text-[12px] text-[#8c8c8c]">Người tham gia: {participantNames}</div>
      </div>
            ...
            <div className="bg-[#f5f5f5] md:w-[140px] lg:w-[150px] shrink-0 p-3 md:border-r border-[var(--voz-border)] flex flex-row md:flex-col items-center md:items-start gap-4 md:gap-0">
               ...
               <div className="flex-1 text-left md:text-center mt-2 w-full">
                  ...
                  <div className="hidden md:flex flex-col items-start w-full text-[11px] text-[#8c8c8c] gap-[2px]">
                     <div className="flex justify-between w-full"><span>Joined</span> <span className="font-medium text-[#141414]">{message.author.createdAt.toLocaleDateString()}</span></div>
                  </div>
               </div>
            </div>
