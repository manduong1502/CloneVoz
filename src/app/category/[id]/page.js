...
export default async function CategoryPage({ params, searchParams }) {
  ...
  if (node.nodeType === 'Category') {
    ...
    const directThreads = await prisma.thread.findMany({
      where: catWhere, orderBy: { [orderField]: orderDir }, skip: catSkip, take: catPerPage,
        ...
        <div className="flex flex-col">
          ...
          <div className="voz-card overflow-visible">
            ...
            <div className="bg-[var(--voz-surface)]">
              {/* Child Forums */}
              {node.children && node.children.length > 0 && node.children.map((child, i) => (
                <div key={child.id} className={`flex items-center p-3 hover:bg-[var(--voz-hover)] transition-colors border-b border-[var(--voz-border-light)]`}>
                  <div className="flex-1 flex items-center min-w-0 pr-2 sm:pr-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex sm:hidden flex-col gap-0.5 mt-1.5">
                        <div className="text-[11px] text-[var(--voz-text-muted)]">Chủ đề: <span className="font-medium text-[var(--voz-text)]">{child.threadsCount || 0}</span> · Bình luận: <span className="font-medium text-[var(--voz-text)]">{child.postsCount || 0}</span></div>
                      </div>
                    </div>
                  </div>
              ...
              {/* Threads */}
              {directThreads.length === 0 && (!node.children || node.children.length === 0) && (
                  ...
                  <div key={thread.id} className="flex py-3.5 px-3 border-b border-[var(--voz-border-light)] hover:bg-[var(--voz-hover)] last:border-0 transition-colors items-start">
                    ...
                    <div className="flex-1 flex flex-col min-w-0 pr-2 md:pr-4">
                      <div className="leading-snug mb-[4px]">
                        ...
                        <Link href={`/thread/${thread.id}`} className="text-[16px] leading-snug font-bold hover:underline thread-title-link"><span>{thread.title}</span></Link>
                      </div>
