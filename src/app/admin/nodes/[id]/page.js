...
export default async function AdminNodeDetail({ params }) {
  ...
  try {
    ...
    return <div className="p-8 text-center text-red-500 font-bold">Lỗi tải dữ liệu: {e.message}</div>;
  }
            ...
            <table className="w-full text-sm">
              <thead className="bg-[var(--voz-hover)] text-[var(--voz-text-muted)] text-xs uppercase">
                <tr>
                  ...
                  <th className="px-4 py-2 text-center w-[80px] hidden sm:table-cell">Bài viết</th>
                  <th className="px-4 py-2 text-center w-[80px] hidden sm:table-cell">Lượt xem</th>
                  <th className="px-4 py-2 text-right w-[140px] hidden md:table-cell">Ngày tạo</th>
              ...
              <tbody className="divide-y divide-[var(--voz-border)]">
                {threads.map(thread => (
                  <tr key={thread.id} className="hover:bg-[var(--voz-hover)] transition-colors">
                    ...
                    <td className="px-4 py-3 text-center text-[var(--voz-text-muted)] hidden sm:table-cell">{thread._count.posts}</td>
                    <td className="px-4 py-3 text-center text-[var(--voz-text-muted)] hidden sm:table-cell">{thread.viewCount}</td>
                    <td className="px-4 py-3 text-right text-[11px] text-[var(--voz-text-muted)] hidden md:table-cell">{formatRelativeTime(thread.createdAt)}</td>
